...
class Notification_Sender:
	...
	def send_password(self, user: User, password: str):
		"""
		Sends a text notification to the user's phone number or email with a temporary password.

		Args:
			user (User): The user receiving the password reset.
			password (str): The temporary password to send.

		Raises:
			TypeError: If the `user` parameter is not an instance of `User`.
		"""
		from src.Service_Layer.User import User
